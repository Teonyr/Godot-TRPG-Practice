...
func get_grid():
	if (_grid == null):
		_grid = _create_grid(grid_size.x, grid_size.y)
	return _grid
...
func register_new (unit : Unit):
	...
	self.grid[current_coordinates.x][current_coordinates.y] = unit.name
	pass
...
func move_request (movement_vector : Vector2, unit : Unit):
	...
	if self.grid[target_coordinates.x][target_coordinates.y] != null:
		return null
...
func _move_unit_in_grid (from : Vector2, to : Vector2):
	...
	self.grid[from.x][from.y] = null
	self.grid[to.x][to.y] = value
